...
proxySp <- newSp %>% filter(is1resptype == 2)
proxy_subset <- proxySp[,grepl( "hc1|ss1|pc1|cp1|mo1|dm1|sc1|ds1|mc1meds|pa1|wb1" , names( proxySp ) )]
proxy_subset <- proxy_subset %>% dplyr::select(-mc1medsrem, -mc1medsdif, -mc1medsyrgo, -mc1medsslf)

spAnal <- proxySp

convertedDistance_Proxy <- daisy(proxy_subset, metric="gower")
#saveRDS(convertedDistance_Proxy, "proxy_subset.rds")
...
ui <- fluidPage(
   ...
   # Application title
   titlePanel("NHATS - Proxy Survey Participants Only (n=583)"),
...
server <- function(input, output) {
  ...
  changeCluster<-reactive({return(pam(convertedDistance_Proxy, k=input$clusters))})
